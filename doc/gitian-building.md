...
*Setup instructions for a gitian build of ARO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ARO
...
Clone the git repositories for apeiron and gitian and then checkout the apeiron version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/AROCRYPTO/ARO.git
cd apeiron
...
Follow the instructions in [doc/release-process.md](release-process.md) in the apeiron repository
...
To build ARO (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the apeiron repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/apeiron/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/AROCRYPTO/ARO
...
./bin/gbuild --commit apeiron=${COMMIT} --url apeiron=${URL} ../apeiron/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit apeiron=${COMMIT} --url apeiron=${URL} ../apeiron/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit apeiron=${COMMIT} --url apeiron=${URL} ../apeiron/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/apeiron-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/apeiron-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/apeiron-build.assert
```
...
[apeiron/gitian.sigs](https://github.com/apeiron-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
